...
public class RabbitFarm<T extends Rabbit> { // Rabbit 타입이면서 Rabbit 하위 타입을 가져옴.

// 클래스와 인터페이스 상속 시 &를 사한다.
// 대신 앞에는 클래스가 와야하고 인터페이스는 뒤에 온다.
// ==> &로 여러 타입을 동시에 가지는 경우에만 타입변수에 맞는 타입으로 여긴다.(둘 중 하나가 아님)
//public class RabbitFarm<T extends Animal & Rabbit> {
//public class RabbitFarm<T extends Rabbit & Animal> {
// ,는 두 개의 타입을 같이 필요로 할 때 사용

// 클래스 여러개는 & 사용이 불가하며, ,(콤마)사용 시 컴파일 에러는 발생하지 않지만
// 뒤에 작성한 클래스를 또 다른 타입변수로 본다. --> 하나의 클래스만 상속받아 구현 할 수 있다.
