...
//        Product product = new Product(); // 객체 생성, 일반 클래스라 잘 됨.

        SmartPhone smartPhone = new SmartPhone();

        System.out.println(smartPhone instanceof SmartPhone); // true // smartPhone은 SmartPhone 타입입니까
        System.out.println(smartPhone instanceof Product); // true

        Product product = new SmartPhone();
        product.abstMethod(); // 재정의된 내용으로 들어옴. 동적 바인딩이 일어남. 상속도 했고 자식쪽에서 재정의도 했다. 즉, 동적바인딩이 됨
