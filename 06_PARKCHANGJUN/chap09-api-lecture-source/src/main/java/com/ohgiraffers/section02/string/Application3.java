...
public class Application3 {
    ...
    public static void main(String[] args) {
        ...
        // 옵션 커맨드 브이 누르면 반환할 것이 뭔지 자동으로 나옴.
        ...
        while (st1.hasMoreTokens()) { // hasMoreToken() : 현재꺼낼 토큰이 있는지없는지 체크
            System.out.println("st1: " + st1.nextToken());
        }
        while (st2.hasMoreTokens()) { // hasMoreToken() : 현재꺼낼 토큰이 있는지없는지 체크
            System.out.println("st2: " + st2.nextToken());
        }
        while (st3.hasMoreTokens()) { // hasMoreToken() : 현재꺼낼 토큰이 있는지없는지 체크
