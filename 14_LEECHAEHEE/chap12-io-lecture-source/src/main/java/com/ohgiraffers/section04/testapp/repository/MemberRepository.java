...
public class MemberRepository {
    ...
    * */
    ...
    public MemberRepository() {
        ...
        if(!file.exists()) {// true : 파일 존재함 false : 파일 없음
            // 파일이 없을때만 새롭게 만들고 추가
            ...
            defaultMembers.add(new Member(2, "user02", "pass02", 10, new String[] {"게임", "만화"}, BloodType.B));
            defaultMembers.add(new Member(3, "user03", "pass03", 15, new String[] {"음악감상", "독서", "명상"}, BloodType.AB));
...
//        }
    ...
    public int insertMember(Member newMember) {
        ...
        try {
            ...
            moo.writeObject(newMember); // 기존 회원에 추가
            // 파일 출력 성공 시 회원을 관리하는 컬렉션에도 추가(최신화)
            memberList.add(newMember);
            // 객체 한개 insert 성공을 의미하는 int값
            result = 1;
        } catch (IOException e) {
            throw new RuntimeException(e);
        } finally {
    ...
    public int deleteMember(int removeMember) {
        ...
        *
        * hard delete를 하게 된다면 memberList에서 회원 한명을 remove()로 제거하고
        * 파일 객체 출력을 통해 파일에 컬렉션에 있는 회원들을 다시 덮어씌워야 한다.(ObjectOutputStream 활용)
        * */
        // soft delete 시
//        int result = -1;
//
