...
import com.ohgiraffers.api.hair.model.dto.HairDTO;
...
public class App {
    public static void main(String[] args) {
        ...
        HairDTO[] hairs = new HairDTO[str.length];

        for (int i = 0; i < str.length; i++) {
            if(i == 0) continue;
            String[] str2 = str[i].split(",");
            long id = Long.parseLong(str2[0]);
            String firstName = str2[1];
            String lastName = str2[2];
            String email = str2[3];
            String gender = str2[4];
            LocalDate birthDate = !"".equals(str2[5]) ? LocalDate.parse(str2[5], DateTimeFormatter.ofPattern("yyyy/MM/dd")) : null;
            LocalDate reservationDate = LocalDate.parse(str2[6]);
            LocalTime reservationTime = LocalTime.parse(str2[7], DateTimeFormatter.ofPattern("H:m"));
            LocalDateTime createdAt = LocalDateTime.parse(str2[8]);

            hairs[i] = new HairDTO(id, firstName, lastName, email, gender, birthDate, reservationDate, reservationTime, createdAt);
            System.out.println(hairs[i].toString());
        }
    }
}
