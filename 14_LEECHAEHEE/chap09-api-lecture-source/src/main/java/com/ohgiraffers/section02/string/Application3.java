...
public class Application3 {
    public static void main(String[] args) {
        ...
        while (st3.hasMoreTokens()) {
            System.out.println("st3: " + st3.nextToken());
        } // 토큰을 꺼낸 이후에는 다시 사용 불가, 재사용하려면 객체 이용해서 다시 잘라내야 함

        String colorStr = "red*orange#blue/yellow green";
//        String[] colors = colorStr.split("#/ ");
        String[] colors = colorStr.split("[*/ #]"); // split()은 대괄호 사용시 해당 문자들이 포함되어 있으면 모두 자름
        System.out.println(Arrays.toString(colors));

        // StringTokenizer의 두번째 인자 문자열 자체는 연속된 문자열이 아닌 하나하나를 구분하겠다는 의미
