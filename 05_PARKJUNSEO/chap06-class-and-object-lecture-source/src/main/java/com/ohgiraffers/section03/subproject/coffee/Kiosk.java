...
public class Kiosk {
    ...
    public void menu () {
        ...
        while (true){
            ...
            switch (choice){
                case "1": createOrder("아메리카노");
                    break;
                case "2": createOrder("라떼");
                    break;
                case "0":
                    return;
                default:
    ...
    private void sendCoffeeRequest(Order order) {
        ...
        System.out.println("> 주문하신 " + Arrays.toString(coffes) + "가 나왔습니다.");
    }

    private Order createOrder(String menuName) {

        String opiton = inputOption();
        int count = inputCount();

        // 주문 생성
        Order order = new Order(menuName, opiton, count);
        // 주문 확인
