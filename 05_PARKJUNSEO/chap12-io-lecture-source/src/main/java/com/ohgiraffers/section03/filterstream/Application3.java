...
public class Application3 {
    public static void main(String[] args) {
        ...
        /*
        * 데이터 자료형별로 처리하는 기능을 추가한 보조스트림을 제공
        ...
        * */
        ...
        try {
            ...
            dout.writeUTF("홍길동");    // IOEXCEPTION에 대한 예외처리 필요
            dout.writeInt(95);
            dout.writeChar('A');
            dout.writeUTF("이순신");    // IOEXCEPTION에 대한 예외처리 필요
            dout.writeInt(85);
            dout.writeChar('B');
            dout.writeUTF("김철수");    // IOEXCEPTION에 대한 예외처리 필요
            dout.writeInt(78);
            dout.writeChar('C');
        ...
        try {
            ...
            while(true) {    // while에 탈출 구문 안 달면 무한루프에 빠져서 java.io.EOFException을 일으킴
                // 파일에 기록한 순서대로 읽어와야 함
                System.out.println(din.readUTF() + ", " + din.readInt() + ", " + din.readChar());    // 읽는 순서를 바꿀 수 없음. 순서를 맞춰줘야 함. 안 그러면 파싱이 잘못됨
            }
        } catch (FileNotFoundException e) {
            throw new RuntimeException(e);
