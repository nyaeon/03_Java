...
public class Application1 {
    public static void main(String[] args) {
        ...
        // 매개변수 없고, 리턴값도 없는 람다  // 실행하고 끝! / 멀티 스레드?
        Runnable runnable = () -> System.out.println("LG TWINS");
        runnable.run();

        /* Consumer<T> #accept(T t) : 객체 T를 받아 소비. 반환 타입 x */
        Consumer<String> consumer = (str) -> System.out.println(str + "이(가) 입력됨" );
        consumer.accept("LG TWINS");

        /* BiConsumer<T, U> #accept(T t, U u) : void => 객체를 T, U로 받아서 소비한다. */
        BiConsumer<String, LocalTime> biConsumer = (str1, time) -> System.out.println(str1 + "이(가)" + time + "에 입력됨");
        biConsumer.accept("LG TWINS", LocalTime.now());
