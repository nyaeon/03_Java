...
public class App {
    public static void main(String[] args) {
        ...
        for (int i = 1; i < uses.length; i++) {
            ...
            String firstName = userObject[1];
            String lastName = userObject[2];
            String email = userObject[3];
            if (email.isEmpty()) {
                email = null;
            }
            String gender = userObject[4];
            if (gender.isEmpty()) {
                gender = null;
            }
            String birthday = userObject[5].replace("/", "-");
            if (birthday.isEmpty()) {
                birthday = null;
            }
            String reservationDate = userObject[6];
            if (reservationDate.isEmpty()) {
                reservationDate = null;
            }
            String resevationTime = userObject[7];
            if (resevationTime.isEmpty()) {
                resevationTime = null;
            }
            String createdAt = userObject[8];
            if (createdAt.isEmpty()) {
                createdAt = null;
            }

            users[i-1] = new User(id, firstName, lastName, email, gender, birthday, reservationDate, resevationTime, createdAt);
        }
