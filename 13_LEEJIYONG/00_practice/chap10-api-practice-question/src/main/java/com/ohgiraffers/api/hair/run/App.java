...
public class App {
    ...
    public static void main(String[] args) {
        ...
        st.nextToken();
        User[] users = new User[st.countTokens()];
        ...
        while (st.hasMoreTokens()) {
            ...
            String email = s[3];
            String gender = s[4];
            LocalDate birth = (s[5].isEmpty()) ? null
                : LocalDate.parse(s[5], DateTimeFormatter.ofPattern("yyyy/MM/dd"));
            LocalDate reservationDate = LocalDate.parse(s[6],
                DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            LocalTime reservationTime = LocalTime.parse(s[7], DateTimeFormatter.ofPattern("H:m"));
            LocalDateTime createAt = LocalDateTime.parse(s[8]);
            users[cnt++] = new User(id, firstName, lastName, email, gender, birth, reservationDate,
                reservationTime, createAt);
        }
