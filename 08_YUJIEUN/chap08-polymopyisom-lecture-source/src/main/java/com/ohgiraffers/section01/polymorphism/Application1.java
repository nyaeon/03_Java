...
//        Tiger tiger1 = new Animal();
        ...
        /*
        * 객체별로 고유한 기능을 동작시키기 위해서는 레퍼런스 변수를 형변환하여 Rabbit 과 Tiger로 변환하여
        * 메소드 호출 가능하다.
        * class type cating : 클래스 형변환
        * ===> 타입 형변환 시 실제 인스턴스와 타입이 일치하지 않는 경우 ClassCastExecption 발생할 수 있다.
        * */
        ((Rabbit) animal1).jump();
        ((Tiger) animal2).bite();
...
//        ((Tiger) animal1).bite();
         ...
         * 또한 작성 여부에 따라 명시적과 묵석 두가지로 구분된다.
         */

        Animal animal3 = (Animal)  new Rabbit(); //업캐스팅(명시적 형변환)
        Animal animal4 = new Rabbit();           //업캐스팅(묵시적 형변솬)

        Rabbit rabbit1 = (Rabbit) animal1;      // Animal => Rabbit 다운캐스팅(명시적 형변환)
//        Rabbit rabbit2 =  animal1;      // 다운캐스팅 묵시적 형변환 안됨
