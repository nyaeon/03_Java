...
public class App {
    public static void main(String[] args) {
        ...
        // CSV 데이터를 줄 단위로 분리 (헤더 포함)
        List<User> users = new ArrayList<>();
        String[] line = data.split("\\R");

        // 날짜 및 시간 파서 설정
        DateTimeFormatter dateFormat = DateTimeFormatter.ofPattern("yyyy-MM-dd");
        // 한 자리 시간도 파싱할 수 있도록 "H:mm" 패턴 사용
        DateTimeFormatter timeFormat = DateTimeFormatter.ofPattern("H:mm");

        // 첫 줄(헤더)은 건너뛰고 나머지 데이터 행을 파싱합니다.
        for (int i = 1; i < line.length; i++) {
            String liner = line[i].trim();
            if (liner.isEmpty()) continue; // 빈 줄 건너뛰기

            // 각 행을 쉼표로 분리
            String[] parts = liner.split(",");
//            if (parts.length < 9) continue; // 데이터 형식 오류 방지
            ...
            // birthday 값이 없으면 null 처리; 값이 있으면 "/"를 "-"로 치환 후 파싱
            LocalDate birthday = null;
            if (!parts[5].isEmpty()) {
                String bday = parts[5].replace("/", "-");
                birthday = LocalDate.parse(bday, dateFormat);
            }

            LocalDate reservationDate = LocalDate.parse(parts[6], dateFormat);
            LocalTime reservationTime = LocalTime.parse(parts[7], timeFormat);
            // created_at은 ISO-8601 형식이므로 기본 파서 사용
