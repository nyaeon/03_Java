...
public class User {
     ...
     * 통상적으로 필드 선언부와 메소드 선언부 사이에 작성하는 것ㄴ이 관례이다.
     * */
...
//    public User(){} // 동일한 이름의 생성자 혹은 메소드를 한 클래스 안에서 작성하는 것은 불가능하다. (compile error)
    ...
    public User(String id, String pwd, String name) { // 매개 변수
        this.id = id; // 인스턴스 변수
        this.pwd = pwd;
        this.name = name;
        System.out.println("User 클래스의 id, pwd, name을 초기화 하는 생성자 호출됨...");
    }
...
//        this.name = name;
        ...
        // this()를 통해서 다른 생성자를 활용할 때는 항상 첫 줄에 위치해야 한다.
        this(id, pwd, name);
