...
//    private final int nonStaticNum;
    ...
    public FinalFieldTest(String nonStaticNMame){
        this.NON_STATIC_NAME = nonStaticNMame; // 자주 사용한다.
    }
...
//    private static final int STATIC_NUM; // 0으로 초기화 되기 때문에 이후 변경 불가능
    ...
    /*
     *  생성자는 인스턴스가 생성되는 시점에 호출되기 때문에 그 전에는 초기화가 일어나지 못 한다.
     * 하지만 static은 프로그램이 start될 때 할당되기 때문에 초기화가 되지 않은 상태로
     * 선언된 것과 동일하며, 기본값으로 초기화 된 후 값을 변경하지 못 하기 때문에 에러가 발생한다.
     * */
