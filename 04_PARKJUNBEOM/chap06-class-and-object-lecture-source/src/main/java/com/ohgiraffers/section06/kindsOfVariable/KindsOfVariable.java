...
public class KindsOfVariable {
    ...
    * 필드 == 멤버변수9클래스가 가지는 멤버라는 뜻) == 전역변수(클래스 전역에서 사용할 수 있는 변수라는 의) == 인스턴스 변수
    * */

    // non-static field : 인스턴스변수(인스턴스 생성 시점에 사용 가능한 변수라는 의미)
    private int globalNum;
    private int num;
    //static field : 정적필드(클래스변수) => static 영역에 생성되는 변수를 의미
...
//        System.out.println(localNum); // 지역변수는 선언 외에 다시 사용(호출)하기 위해서는 반드시 초기화가 되어야한다.
        ...
        System.out.println(this.num); // 전역변수 num 출력 this로 전역변수와 지역변수 구분 (지역변수 > 전역변수)
    } // 메소드 영역의 끝
