...
public class FinalFieldTest {
     ...
     * */
    /*
    * final은 변경불가의 의미를 가진다.
    * 따라서 초기 인스턴스가 생성되고 나면 기본값이 0인 필드에 들어가게 되는데
    * 그 초기화 이후 값을 변경할 수 없기 때문에 선언하면서 바로 초기화를 해주어야한다.
    **/
//    private final int nonStaticCount = 0;
    // 해결방법
    // 1. 선언과 동시에 초기화 한다.
...
//    private static final int STATIC_NUM; // 0으로 초기화 되기 때문에 이후 변경 불가능
    ...
    /*
    * 생성자는 인스턴스가 생서되는 시점에 호출되기 때문에 그전에는 초기화가 일어나지 못한다.
    * 하지만 static은 프로그램이 시작될 때 할당되기 때문에 최화가 되지 않은 상태로
    * 선언된 것과 동일하며 기봇값으로 초기화된 후 값을 변경하지 못하기 때문에 에러가 발생한다.
    * */
