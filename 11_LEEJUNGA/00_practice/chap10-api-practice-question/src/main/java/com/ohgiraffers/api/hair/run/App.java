...
public class App {
    public static void main(String[] args) {
        ...
        while (st.hasMoreTokens()) {
            String[] userInfos = st.nextToken().split(",");
            User user = new User(
                    Integer.parseInt(userInfos[0]),
                    userInfos[1],
                    userInfos[2],
                    userInfos[3],
                    userInfos[4],
                    userInfos[5].isEmpty() ? null : LocalDate.parse(userInfos[5], DateTimeFormatter.ofPattern("yyyy/MM/dd")),
                    LocalDate.parse(userInfos[6]),
                    LocalTime.parse(userInfos[7].length() == 4 ? "0" + userInfos[7] : userInfos[7]),
                    LocalDateTime.now()
            );
