...
public class Application2 {
    public static void main(String[] args) {
         ...
         * 2. Object의 colne()을 이용한 복사(사용 빈도 높음)
        ...
        copyArr1[0] = 100;
        print(originArr, "originArr 확인용"); // 넘어온 배열의 hashCode 값 : 1670782018
        print(copyArr1, "copyArr1 확인용"); // 넘어온 배열의 hashCode 값 : 834600351

        // 2. Object의 colne()을 이용한 복사(사용 빈도 높음)
        int[] copyArr2 = originArr.clone();
        print(copyArr2, "clone()을 활용한 사본"); // 배열의 주소값이 다름

        // 3. System의 arraycopy()를 이용한 복사
        int[] copyArr3 = new int[originArr.length + 3]; // length = 7

        // [복사할 원본 배열], [복사를 시작할 인덱스] ,[복사본 배열], [복사를 시작할 인덱스], [복사할 길이]
        System.arraycopy(originArr, 0, copyArr3, 0, originArr.length);
        print(copyArr3, "System.arraycopy()를 활용한 사본");

        // 4. Arrays의 copyOf()를 이용한 복사 (원본의 처음부터만 가능)
