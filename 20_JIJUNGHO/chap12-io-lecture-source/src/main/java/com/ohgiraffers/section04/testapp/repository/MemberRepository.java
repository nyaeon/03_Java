...
public class MemberRepository {
    ...
    public MemberRepository() {
        ...
        // main을 실행할 때마다 덮어씌우는 문제를 방지하고자, 파일이 없을때만 초기 세팅이 되도록 조건문을 작성
        if (!file.exists()) {
            // 파일이 해당 경로에 없을 경우이면 새롭게 파일 추가
            // 배열 형식으로 사용하기 위해 ArrayList 사용
            // <Member> Member 타입의 객체만 사용 가능 (memNo, id, pwd, age, honnies, bloodType)
            ...
            defaultMembers.add(new Member(2, "user02", "pass02", 10, new String[]{"태권도", "게임"}, BloodType.AB));
            defaultMembers.add(new Member(3, "user03", "pass03", 15, new String[]{"만화", "탁구", "명상"}, BloodType.B));

            SaveMember(file, defaultMembers);
        }

        // 파일로부터 회원 객체들을 입력받아 memberList에 쌓기
        loadMember(file);

        // 값이 정상적으로 들어왔는지 확인용 코드
     ...
     */
    private void SaveMember(File file, ArrayList<Member> members) {
        // inpuStream : 파일안에 정보를 읽어오는것
        // outputStream : 파일을 생성하는것
        ObjectOutputStream oos = null;

        try {
            oos = new ObjectOutputStream(new BufferedOutputStream(new FileOutputStream(file)));
            for (Member member : members) {
                oos.writeObject(member); // 파일에 값 추가
            }
        } catch (IOException e) {
            throw new RuntimeException(e);
        } finally {
    ...
    public int selectLastMemberNo() {
        ...
        // 컬렉션의 크기가 곧 회원번호이므로 마지막 회원의 번호는 컬렉션의 크기(항상 맞지는x)
    ...
    public int deleteMember(int removeMember) {
         ...
         *
         * hard delete를 하게 된다면 memberList에서 회원 한명을 renmove()메소드를 활용하여 삭제하고
