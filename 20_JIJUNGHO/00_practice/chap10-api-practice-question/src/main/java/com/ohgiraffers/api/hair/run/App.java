...
import com.ohgiraffers.api.hair.resvDTO.UserDTO;
...
public class App {
    public static void main(String[] args) {
        ...
        // 데이터를 개행 구분자로 split 처리 한 데이터를 splitData에 담음
        String[] splitData = data.split("\n");
        // userDTO에 split 처리한 데이터의 길이 -1 만큼 저장 why? 첫줄이 데이터가 아닌 한줄이 있기 때문에
        UserDTO[] userDto = new UserDTO[splitData.length - 1];

        // splitData의 길이 만큼 반복문을 돌며, 데이터를 타입에 맞게 파싱해서 저장
        for (int i = 1; i < splitData.length; i++) {
            String[] regexData = splitData[i].split(",", -1);

            int id = Integer.parseInt(regexData[0]);
            String firstName = regexData[1];
            String lastName = regexData[2];
            String email = regexData[3];
            char gender = regexData[4].charAt(0);
            LocalDate birthday = null;

            if (!regexData[5].isEmpty()) {
                birthday = LocalDate.parse(regexData[5].replace("/", "-"));
            }
            DateTimeFormatter dateFormat = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            DateTimeFormatter timeFormatter = DateTimeFormatter.ofPattern("H:mm");
            DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm:ss");

            LocalDate reservationDate = LocalDate.parse(regexData[6],dateFormat);
            LocalTime reservationTime = LocalTime.parse(regexData[7], timeFormatter);
            LocalDateTime createdAt = LocalDateTime.parse(regexData[8], dateTimeFormatter);

            userDto[i - 1] = new UserDTO(id, firstName, lastName, email, gender, birthday, reservationDate, reservationTime, createdAt);
        }
