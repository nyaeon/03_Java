...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("suject1 = " + subject1);
        Subjects subject2 = Subjects.HTML;
        System.out.println("suject2 = " + subject2);
        Subjects subject3 = Subjects.JAVA;
        System.out.println("suject3 = " + subject3);

        /*
         * 1. 열거타입으로 선언된 인스턴스는 싱글톤으로 관리되며 인스턴스가 각각 한 개임을 보장한다.
        ...
        // 단일 인스턴스임을 보장히기에 == 비교가 가능하다.(동일 객체 비교가 가능하다.)
        if (subject1 == subject3) {
            System.out.println("싱글톤이다.");
        }

        // 오버라이딩 되지 않은 toString() 또는 name() 메소드를 활용하여 필드명을 문자열로 변경하기 쉽다.
        System.out.println(Subjects.JAVA); // toString()을 자동 적용한 채 출력
        System.out.println(Subjects.JAVA.name()); // 해당 값 출력

        // 이름 충돌 방지를 위해 접두사를 쓰지 않아도 Enum타입별로 네임스페이스를 가져간다.
        ...
        for (Subjects subject : subjects) {
            ...
            System.out.println(subject.ordinal()); // 순회하는 값의 순번을 출력
        }
