...
public class MemberRepository {
    ...
    public MemberRepository() {
        ...
        if(!file.exists()){ // exists()의 동작 >> true: 파일이 있을때 false : 파일이 없을 때
            ...
            dafaultMembers.add(new Member( 2, "user02", "pass02", 20, new String [] {"게임", "수영"} , BloodType.B ));
            dafaultMembers.add(new Member( 3, "user03", "pass03", 20, new String [] {"음악감상", "독서"} , BloodType.AB ));
...
//        }
    ...
    private void loadMember2(File file) {
        try {
...
//        return memberList.size();
    ...
    public int deleteMember(int removeMember) {
         ...
         * soft delte(회원 중에 탈퇴와 관련된 속성 수정)을 진행해야 한다.
         *
         * hard delte를 하게 된다면 memberList에서 회원 한명을 remove()메소드를 활용하여 삭제하고
         * 파일 객체 총력을 통해 파일에 컬렉션에 있는 회원들을 다시 덮어씌워야 한다. (ObjectOutputStream을 활용해서)
         */
        // soft delete 시
//        int result = -1;
//
...
//        return result;
        ...
        // hard delete 시
        for(int i = 0; 1 < memberList.size(); i++) {
