...
public class App {
    ...
    public static void main(String[] args) {
        ...
        // CSV 데이터 파싱 및 DTO 객체 생성
        List<User> revervations = parseCSVData(data);
        printReservations(revervations);
    }

    private static List<User> parseCSVData(String data) {
        ...
        for(int i = 1; i < lines.length; i++) {
            ...
            // birthday 설정(비어있을수도 있음)
            if (!fields[5].isEmpty()) {
                DateTimeFormatter birthFormat = DateTimeFormatter.ofPattern("yyyy/MM/dd");
                LocalDate birthDate = LocalDate.parse(fields[5], birthFormat);
                user.setBirthday(birthDate);
            }

            // reservationDate 설정
            DateTimeFormatter dateFormat = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            LocalDate date = LocalDate.parse(fields[6], dateFormat);
            user.setReservationDate(date);

            // reservationTime 설정
            DateTimeFormatter timeFormat = DateTimeFormatter.ofPattern("H:mm");
            LocalTime reservationTime = LocalTime.parse(fields[7], timeFormat);
            user.setReservationTime(reservationTime);

            // createdAt 설정
            DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ISO_LOCAL_DATE_TIME;
            LocalDateTime createdAt = LocalDateTime.parse(fields[8], dateTimeFormatter);
    ...
    private static void printReservations(List<User> reservations) {
        System.out.println("===== 예약 정보 =====");
        for (User reservation : reservations) {
            System.out.println(reservation);
        }
        System.out.println("총 " + reservations.size() + "개의 예약 정보가 출력되었습니다.");
