...
import com.ohgiraffers.api.hair.dto.Member;
...
public class App {
    public static void main(String[] args) {
        ...
        String[] memberStr = new String[101];
        String[] newStr = new String[100];
        memberStr=data.split("\n");
        for(int i=0;i<memberStr.length-1;i++) {
           newStr[i]=memberStr[i+1];
            System.out.println(newStr[i]);
        }
        for(int i=0;i<newStr.length;i++) {
            String[] strAtline = new String[9];
            strAtline=newStr[i].split(",",-1);
            if(strAtline[7].length()== 4) {
                strAtline[7]="0"+strAtline[7];
            }
            System.out.println(strAtline[8]);
            LocalDate birth = strAtline[5].isBlank() ? null : LocalDate.parse(strAtline[5], DateTimeFormatter.ofPattern("yyyy/MM/dd"));
            LocalDate reservation_date = LocalDate.parse(strAtline[6], DateTimeFormatter.ofPattern("yyyy-MM-dd"));
            LocalTime reservation_time = LocalTime.parse(strAtline[7], DateTimeFormatter.ofPattern("HH:mm"));

            LocalDateTime createdAt = LocalDateTime.parse(strAtline[8]);

           members[i] = new Member(Integer.parseInt(strAtline[0]),strAtline[1],strAtline[2],strAtline[3],strAtline[4],birth,reservation_date,reservation_time,createdAt);
