...
public class App {
    public static void main(String[] args) {
        ...
        for(int i = 1; i < temp.length; i++) {
            String[] sarr = temp[i].split(",");
            long id = Integer.parseInt(sarr[0]);
            String firstName = sarr[1];
            String lastName = sarr[2];
            String email = sarr[3];
            String gender = sarr[4];
            LocalDate birthday = sarr[5].equals("") ? null : LocalDate.parse(sarr[5], DateTimeFormatter.ofPattern("yyyy/MM/dd"));
            LocalDate reservationDate = LocalDate.parse(sarr[6]);
            LocalTime reservationTime = LocalTime.parse(sarr[7], DateTimeFormatter.ofPattern("H:m"));
            LocalDateTime createdAt = LocalDateTime.parse(sarr[8]);
