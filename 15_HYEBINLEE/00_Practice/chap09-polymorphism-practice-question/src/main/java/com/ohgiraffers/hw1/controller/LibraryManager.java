...
public class LibraryManager{
  ...
  public int rentBook(int index) {
    int result = 0; // 0: 만화책 대여 가능, 1: 대여 불가능, 2: 요리책 대여 가능 & 쿠폰 발급
    Book myBook = bList[index];
    if(myBook instanceof AniBook) {
      AniBook aniBook = (AniBook) myBook;
      if(mem.getAge() < (aniBook.getAccessAge())){
        result = 1;
      }
      else{
        CookBook cookBook = (CookBook) myBook;
